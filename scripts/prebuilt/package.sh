...
# contains recipe-key virtual entries only; resolver decisions are based on
# recipe state, while archive transport failures are handled by the caller.
#
# Inputs (in cwd):
#   raw guest artifacts listed by scripts/prebuilt/artifact-inputs.sh
#   plus the source inputs listed by scripts/prebuilt/artifact-inputs.sh
#
# Outputs (in cwd):
...
require_raw_artifacts() {
    ...
    while IFS= read -r class; do
        while IFS= read -r artifact; do
            if [ ! -f "$artifact" ]; then
                echo "[!] Missing $artifact -- materialize it via make or scripts/prebuilt/plan-materialize.sh first" >&2
                exit 1
            fi
...
while IFS= read -r class; do
    ...
    class_recipe_keys+=("$(prebuilt_class_recipe_key "$class")")
done < <(prebuilt_classes)

{
    index=0
    while IFS= read -r class; do
        write_manifest_entries_for_class "$class" "${class_recipe_keys[$index]}"
        index=$((index + 1))
    done < <(prebuilt_classes)
} > prebuilt.sha1
