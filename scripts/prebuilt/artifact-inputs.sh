...
# Provider-neutral metadata shared by resolver, materializer, packager,
# stamp helper, and Make diagnostics.  The registry intentionally contains
# only stable class identity: user-facing class name, KEY=VALUE plan prefix,
# and raw artifact filenames.  Recipe inputs stay in prebuilt_class_inputs()
# because they are easier to review as explicit per-class lists.
#
    ...
    case "$1" in
        image)
            printf '%s\n' \
                ...
                scripts/prebuilt/artifact-recipe-buildroot.env \
                scripts/prebuilt/artifact-recipe-linux.env \
                scripts/prebuilt/build-artifacts.sh \
                scripts/prebuilt/build-plan-artifacts.sh
            ;;
        rootfs)
            printf '%s\n' \
                configs/buildroot.config \
        ...
        test-tools)
            printf '%s\n' \
                ...
                scripts/prebuilt/artifact-recipe-buildroot.env \
                scripts/prebuilt/artifact-recipe-test-tools.env \
                scripts/prebuilt/build-artifacts.sh \
                scripts/prebuilt/build-plan-artifacts.sh \
...
Commands:
  ...
  inputs <class>    List files that define one class recipe key.
