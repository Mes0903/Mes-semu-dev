...
bool vgpu_display_pop_cmd(struct vgpu_display_cmd *cmd)
{
     ...
     */
    for (;;) {
        /* Check clear command for primary and cusor plane. */
        ...
        /* Pop the command and check if it's still valid */
...
void vgpu_display_set_unavailable(void)
{
    ...
    /* In the current tree this latch is set only when window-sw initialization
     * fails and semu falls back to headless mode, before the emulator thread is
     * started. Still publish it atomically so later call sites keep the same
