...
static bool virtio_input_capture_common_generation(
    virtio_input_state_t *vinput,
    uint64_t *generation)
{
    unsigned status;
...
static bool virtio_input_begin_actor_completion(
    virtio_input_state_t *vinput,
                                      ...
                                      uint64_t common_generation)
{
    ...
    for (;;) {
        ...
        if (!virtio_input_actor_generation_current(vinput, actor,
                                                   actor_generation))
            return 0;
        if (!virtio_input_common_generation_current(vinput,
                                                    common_generation))
            return 0;
    ...
    if (consumed &&
        virtio_input_begin_actor_completion(vinput, actor_generation,
                                            common_generation)) {
        ...
        while (vinput_pop_cmd(dev_id, &event)) {
            ...
            case VINPUT_CMD_KEYBOARD_KEY:
                if (dev_id == VINPUT_KEYBOARD_ID)
                    virtio_input_update_key(actor, actor_generation,
                                            common_generation,
                                            event.u.keyboard_key.key,
            ...
            case VINPUT_CMD_MOUSE_WHEEL:
                if (dev_id == VINPUT_MOUSE_ID)
                    virtio_input_update_scroll(actor, actor_generation,
                                               common_generation,
                                               event.u.mouse_wheel.dx,
                                          ...
                                          uint64_t generation)
{
    ...
    if (!vinput ||
        (queue_index != VIRTIO_INPUT_EVENTQ &&
    ...
    if (queue_index == VIRTIO_INPUT_EVENTQ) {
        virtio_input_drain_host_events_for_actor(
            vinput, actor, generation, common_generation);
        return 0;
    }
