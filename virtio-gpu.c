...
static struct virtio_gpu_config virtio_gpu_config;
                                   ...
                                   uint32_t size)
{
    if (addr >= RAM_SIZE || size > RAM_SIZE || addr + size > RAM_SIZE) {
        fprintf(stderr,
                "virtio-gpu: guest address 0x%x size 0x%x out of bounds\n",
                addr, size);
        return NULL;
    }
...
size_t virtio_gpu_iov_to_buf(const struct iovec *iov,
                             ...
                             size_t bytes)
{
    ...
    /* Each iovec entry describes one contiguous chunk of the guest backing.
     * Treat the array as one long byte stream: skip whole entries until
     * reaching the starting offset, then memcpy each chunk into buf until the
     * requested byte count is collected.
     */
...
uint32_t virtio_gpu_write_response(virtio_gpu_state_t *vgpu,
                                   ...
                                   uint32_t type)
{
    if (desc_len < sizeof(struct virtio_gpu_ctrl_hdr))
        return 0;

    struct virtio_gpu_ctrl_hdr *response = virtio_gpu_mem_guest_to_host(
        vgpu, addr, sizeof(struct virtio_gpu_ctrl_hdr));
    if (!response)
        return 0;
                                     ...
                                     uint32_t desc_len)
{
    ...
    /* Guest is riscv32, upper 32 bits of addr are always 0. */
    struct virtio_gpu_ctrl_hdr *response = virtio_gpu_mem_guest_to_host(
        vgpu, (uint32_t) addr, sizeof(struct virtio_gpu_ctrl_hdr));
    if (!response)
        return;
...
void virtio_gpu_get_display_info_handler(virtio_gpu_state_t *vgpu,
                                         ...
                                         uint32_t *plen)
{
    if (vq_desc[0].len < sizeof(struct virtio_gpu_ctrl_hdr)) {
        *plen = 0;
        return;
    }

    struct virtio_gpu_ctrl_hdr *request = virtio_gpu_mem_guest_to_host(
        vgpu, vq_desc[0].addr, sizeof(struct virtio_gpu_ctrl_hdr));
    if (!request) {
        *plen = 0;
        return;
    }
    ...
    if (vq_desc[resp_idx].len < sizeof(struct virtio_gpu_resp_disp_info)) {
        *plen = 0;
        return;
    }
     ...
     * pmodes[i] -> scanout_id i model because later requests only carry a
     * scanout_id, and Linux does the same when it copies resp->pmodes[i] into
     * outputs[i] and later sends output->index in SET_SCANOUT. See
     * virtgpu_vq.c and virtgpu_display.c for more details.
     */
    ...
    *plen = sizeof(*response);
    ...
    virtio_gpu_set_response_fencing(vgpu, request, vq_desc[resp_idx].addr,
                                 ...
                                 uint32_t *plen)
{
    ...
    if (vq_desc[resp_idx].len < sizeof(struct virtio_gpu_resp_edid)) {
        *plen = 0;
        return;
    }
                                      ...
                                      uint32_t *plen)
{
    ...
    struct virtio_gpu_ctrl_hdr *header = virtio_gpu_mem_guest_to_host(
        vgpu, vq_desc[0].addr, sizeof(struct virtio_gpu_ctrl_hdr));
    if (!header) {
        virtio_gpu_set_fail(vgpu);
        *plen = 0;
                                     ...
                                     uint32_t height)
{
    ...
    virtio_gpu_config.num_scanouts = PRIV(vgpu)->num_scanouts;
