...
void *vgpu_mem_guest_to_host(virtio_gpu_state_t *vgpu,
                             uint32_t addr,
                             uint32_t size)
{
...
uint32_t virtio_gpu_write_response(virtio_gpu_state_t *vgpu,
                                   ...
                                   uint32_t type)
{
    if (desc_len < sizeof(struct vgpu_ctrl_hdr))
        return 0;

    struct vgpu_ctrl_hdr *response =
        vgpu_mem_guest_to_host(vgpu, addr, sizeof(struct vgpu_ctrl_hdr));
    if (!response)
        return 0;
...
static void virtio_gpu_update_status(virtio_gpu_state_t *vgpu, uint32_t status)
{
     ...
     * It holds only host-configured scanout info (display dimensions /
     * enabled flags) set up before the guest driver probes the device.
     * The guest re-queries this via CMD_GET_DISPLAY_INFO after each
     * reset, so it must survive. Runtime state (resource-to-scanout
     * mapping) lives in vgpu_res_2d_list and is freed below.
     *
     * If runtime per-scanout state is ever added to virtio_gpu_data_t,
     * revisit this.
     */
                                     ...
                                     uint32_t desc_len)
{
    ...
    struct vgpu_ctrl_hdr *response = vgpu_mem_guest_to_host(
        vgpu, (uint32_t) addr, sizeof(struct vgpu_ctrl_hdr));
    if (!response)
        return;
                                         ...
                                         uint32_t *plen)
{
    ...
    if (vq_desc[resp_idx].len < sizeof(struct vgpu_resp_disp_info)) {
        *plen = 0;
        return;
    }
                                 ...
                                 uint32_t *plen)
{
    ...
    /* Write EDID response */
    if (vq_desc[resp_idx].len < sizeof(struct vgpu_resp_edid)) {
        *plen = 0;
        return;
    }
...
void virtio_gpu_cmd_undefined_handler(virtio_gpu_state_t *vgpu,
                                      ...
                                      uint32_t *plen)
{
    if (vq_desc[0].len < sizeof(struct vgpu_ctrl_hdr)) {
        virtio_gpu_set_fail(vgpu);
        *plen = 0;
        return;
    }

    struct vgpu_ctrl_hdr *header = vgpu_mem_guest_to_host(
        vgpu, vq_desc[0].addr, sizeof(struct vgpu_ctrl_hdr));
    if (!header) {
        virtio_gpu_set_fail(vgpu);
        *plen = 0;
        return;
...
static void virtio_queue_notify_handler(virtio_gpu_state_t *vgpu, int index)
{
    ...
    vgpu->ram[queue->QueueUsed] &= MASK(16); /* clear high 16 bits (idx) */
    vgpu->ram[queue->QueueUsed] |= ((uint32_t) new_used) << 16; /* set idx */
