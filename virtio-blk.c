                                        ...
                                        uint64_t disk_offset)
{
    ...
    for (size_t i = 0; i < count; i++) {
        int ret = virtio_blk_copy_disk_to_guest(vblk, iov[i].addr, disk,
                                                iov[i].len);
                                         ...
                                         uint64_t disk_offset)
{
    ...
    for (size_t i = 0; i < count; i++) {
        int ret = virtio_blk_copy_guest_to_disk(vblk, iov[i].addr, disk,
                                                iov[i].len);
    ...
    *used_len = 0;
    ...
    if (header.type == VIRTIO_BLK_T_IN) {
        ...
        if (ret == -EFAULT)
            return ret;
        if (ret < 0) {
            if (!virtio_blk_write_status(vblk, status_iov,
                                         VIRTIO_BLK_S_IOERR))
                return -EFAULT;
            return 0;
        }
