...
| Actor invariant failure or OOM | Mark the device failed, wake synchronous waiters, set `DEVICE_NEEDS_RESET`, and transition the actor to `FAILED`. VM `FAILED` is reserved for essential device failure or possible memory corruption. | `virtio_actor_fail()` broadcasts waiters, then calls the actor `on_failed` hook outside the actor lock; wait/callback tests live in `tests/test-virtio-actor.c`. |
