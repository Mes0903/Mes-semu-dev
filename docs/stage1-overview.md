...
│    ├── 0x1F700000  initrd (初始 RAM 磁碟)          │
│    └── 0x1FF00000  設備樹 DTB                       │
│ 0x07FFFFFF (128MB 上限)                             │
├─────────────────────────────────────────────────────┤
│ 0xF0000000                                          │
│ MMIO 設備區域                                       │
│    ├── 0xF0000000  PLIC (中斷控制器, 64MB)          │
│    ├── 0xF4000000  UART (串口, 1MB)                 │
│    ├── 0xF4100000  VirtIO-Net (網路, 1MB)           │
│    ├── 0xF4200000  VirtIO-Blk (磁碟, 1MB)           │
│    ├── 0xF4300000  ACLINT MTIMER (計時器, 1MB)      │
│    ├── 0xF4400000  ACLINT MSWI (M軟體中斷, 1MB)     │
│    ├── 0xF4500000  ACLINT SSWI (S軟體中斷, 1MB)     │
│    ├── 0xF4600000  VirtIO-RNG (隨機數, 1MB)         │
│    ├── 0xF4700000  VirtIO-Snd (音效, 1MB)           │
│    └── 0xF4800000  VirtIO-FS (文件系統, 1MB)        │
│ 0xFFFFFFFF                                          │
└─────────────────────────────────────────────────────┘
```

**關鍵代碼**：[main.c](../main.c):204-267 的 `mem_load` 函數 (static 函數)
...
bool semu_init(emu_state_t *emu, config_t *config) {
    ...
    for (int i = 0; i < config->n_hart; i++) {
        ...
        // 設置記憶體回調
        hart->mem_fetch = mem_fetch;      // main.c:181 (static 函數)
        hart->mem_load = mem_load;        // main.c:204 (static 函數)
        hart->mem_store = mem_store;      // main.c:269 (static 函數)
        hart->mem_page_table = mem_page_table;  // main.c:365 (static 函數)
        hart->wfi = wfi_handler;          // WFI 協程回調
...
**位置**：[main.c](../main.c):204-267 (static 函數 `mem_load`)

```c
static void mem_load(hart_t *hart, uint32_t addr, uint8_t width, uint32_t *value)
{
    emu_state_t *data = PRIV(hart);  // PRIV 宏展開: ((emu_state_t *)hart->priv)
    ...
    // ═══════════════════════════════════
    // MMIO 設備區域 (0xF0000000 - 0xFFFFFFFF)
    // ═══════════════════════════════════
...
**地址解碼邏輯**：
```c
// 示例: 地址 0xF4300000 (MTIMER)
addr = 0xF4300000

// 步驟 1: 檢查高 4 位是否為 0xF
(addr >> 28) == 0xF  // true (0xF4300000 >> 28 = 0xF)

// 步驟 2: 提取第 20-27 位作為區域索引
(addr >> 20) & MASK(8)  // MASK(8) = 0xFF
// 0xF4300000 >> 20 = 0xF43
// 0xF43 & 0xFF = 0x43
// 因此進入 case 0x43 (MTIMER)

// 步驟 3: 提取低 20 位作為設備內偏移
addr & 0xFFFFF  // 0xF4300000 & 0xFFFFF = 0x00000
// 傳遞給 aclint_mtimer_read 作為相對偏移
```

**設計要點**：
1. **統一介面**：所有設備使用相同的 `read(hart, state, offset, width, value)` 介面
2. **快速路由**：使用位移和掩碼快速解碼地址 (避免多次 if 比較)
3. **錯誤處理**：未映射地址觸發 Load Fault 異常
4. **條件編譯**：設備支援根據編譯選項 (#if SEMU_HAS(...)) 決定
5. **1MB 區域**：每個設備分配 1MB 地址空間 (實際使用遠小於此)
...
2. **Q**: 記憶體映射中，RAM 默認佔用哪段地址？
   **A**: 0x00000000 - 0x07FFFFFF (128MB 默認, 可配置)

3. **Q**: 內核映像載入到哪個地址？
   **A**: 0x00000000

4. **Q**: 設備樹 DTB 載入到哪個地址？
   **A**: 0x1FF00000 (相對 RAM 開始的偏移)

5. **Q**: UART 映射到哪個地址？
   **A**: 0xF4000000

6. **Q**: ACLINT MTIMER 映射到哪個地址？
   **A**: 0xF4300000
...
4. **Q**: `mem_load` 如何快速解碼 MMIO 地址？
   **A**: 使用位移和掩碼: `(addr >> 20) & MASK(8)` 提取區域索引，避免多次 if 比較

5. **Q**: 記憶體訪問如何路由到不同設備？
   **A**: 通過 `mem_load/mem_store` 函數 (main.c 中的 static 函數)，根據地址範圍分發到對應設備的讀寫函數
