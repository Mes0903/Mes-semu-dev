...
#if SEMU_HAS(VIRTIONET)
    EMU_DEVICE_CALL(emu->vnet_lock, if (refresh_net) {
...
#if SEMU_HAS(VIRTIOINPUT) || SEMU_HAS(VIRTIOGPU)
    bool window_ready = g_window.window_init(headless, SCREEN_WIDTH,
                                             SCREEN_HEIGHT);
#if SEMU_HAS(VIRTIOGPU)
    if (!window_ready)
...
#if SEMU_HAS(VIRTIONET)
    EMU_DEVICE_CALL(emu->vnet_lock,
                    virtio_net_event_poll_fallback(&emu->vnet));
#else
    (void) emu;
#endif
}
