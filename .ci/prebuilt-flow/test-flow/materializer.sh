...
test_materialize_downloads_release_and_writes_stamps() {
    ...
    (
        ...
        PREBUILT_URL="file://$release" .ci/prebuilt/plan-materialize.sh > "$output" 2> "$err"
        PREBUILT_VERBOSE=1 PREBUILT_URL="file://$release" .ci/prebuilt/plan-materialize.sh > "$tmp/materialize-verbose.out" 2> "$tmp/materialize-verbose.err"
    )
...
test_materialize_consumes_supplied_plan_without_resolving_again() {
    ...
    make_materializer_fixture "$fixture"
    cat > "$fixture/.ci/prebuilt/build-plan-artifacts.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
    chmod +x "$fixture/.ci/prebuilt/build-plan-artifacts.sh"
    cat > "$fixture/.ci/prebuilt/resolve-artifacts.sh" <<'EOF'
#!/usr/bin/env bash
echo "resolver should not run when PREBUILT_PLAN_FILE is set" >&2
exit 99
EOF
    chmod +x "$fixture/.ci/prebuilt/resolve-artifacts.sh"
    cat > "$plan" <<EOF
plan_version=1
image_action=build
...
EOF
    ...
    (
        ...
        PREBUILT_URL="file://$fixture/missing-release" \
            ...
            .ci/prebuilt/materialize-artifacts.sh > "$output"
    )
...
EOF
    ...
    if (
        ...
        PREBUILT_URL="file://$fixture/missing-release" \
            PREBUILT_PLAN_FILE="$plan" \
            .ci/prebuilt/materialize-artifacts.sh image > "$output" 2>&1
    ); then
...
test_materialize_requires_supplied_plan() {
    ...
    if (
        cd "$fixture"
        PREBUILT_URL="file://$fixture/missing-release" \
            .ci/prebuilt/materialize-artifacts.sh > "$output" 2>&1
    ); then
...
test_materialize_rejects_stamp_actions_without_current_recipe_key() {
    local fixture=$tmp/materialize-missing-current-key
    local release=$tmp/materialize-missing-current-key-release
    local output=$tmp/materialize-missing-current-key.out
    local plan=$tmp/materialize-missing-current-key.env
    local action

    for action in build download-release; do
        fixture=$tmp/materialize-missing-current-key-$action
        output=$tmp/materialize-missing-current-key-$action.out
        plan=$tmp/materialize-missing-current-key-$action.env
        make_materializer_fixture "$fixture"
        write_release_with_archives "$release-$action" \
...
rootfs_action=use-local
test_tools_action=use-local
current_rootfs_recipe_key=2222222222222222222222222222222222222222
current_test_tools_recipe_key=3333333333333333333333333333333333333333
EOF

        if (
            cd "$fixture"
            PREBUILT_URL="file://$release-$action" \
                PREBUILT_PLAN_FILE="$plan" \
                .ci/prebuilt/materialize-artifacts.sh > "$output" 2>&1
        ); then
            echo "[!] materializer unexpectedly accepted $action without current image key" >&2
            exit 1
        fi
        assert_contains "$output" "Missing current recipe key for prebuilt class image with action $action"
    done
}
...
test_materialize_rejects_build_actions_when_builds_forbidden() {
    local fixture=$tmp/materialize-forbid-build
    local output=$tmp/materialize-forbid-build.out
    local plan=$tmp/materialize-forbid-build.env
    local image_key=1111111111111111111111111111111111111111
    local rootfs_key=2222222222222222222222222222222222222222
    local test_tools_key=3333333333333333333333333333333333333333
...
plan_version=1
image_action=build
rootfs_action=download-release
test_tools_action=skip
current_image_recipe_key=$image_key
current_rootfs_recipe_key=$rootfs_key
current_test_tools_recipe_key=$test_tools_key
...
EOF
    ...
    if (
        ...
        echo "[!] materializer unexpectedly accepted a build action with PREBUILT_FORBID_BUILD=1" >&2
        exit 1
    fi

    test ! -e "$fixture/forbidden-build.args"
    assert_contains "$output" 'PREBUILT_FORBID_BUILD forbids source builds'
...
test_materialize_accepts_recipe_only_release_manifest() {
    ...
    (
        ...
        rm -f Image rootfs.cpio test-tools.img
        PREBUILT_URL="file://$release" .ci/prebuilt/plan-materialize.sh > "$output"
    )
...
test_materialize_requested_classes_only() {
    ...
    (
        ...
        write_release_with_archives "$release" "$image_key" "$rootfs_key" "$test_tools_key"
        rm -f Image rootfs.cpio test-tools.img
        PREBUILT_URL="file://$release" .ci/prebuilt/plan-materialize.sh image rootfs > "$output"
    )
...
test_resolver_and_materializer_run_from_outside_repo_root() {
    ...
    (cd "$tmp" && PREBUILT_URL="file://$release" "$fixture/.ci/prebuilt/resolve-artifacts.sh" > "$output")
    assert_contains "$output" '^image_action=download-release$'

    (cd "$tmp" && PREBUILT_URL="file://$release" "$fixture/.ci/prebuilt/plan-materialize.sh" > "$output")
    assert_contains "$fixture/Image" '^release kernel$'
    assert_contains "$fixture/rootfs.cpio" '^release rootfs$'
    assert_contains "$fixture/test-tools.img" '^release tools$'
}
