...
test_package_and_verify() {
    ...
    (
        cd "$fixture"
        scripts/prebuilt/package.sh > "$output" 2> "$tmp/package.err"
        scripts/prebuilt/verify-package.sh > "$tmp/verify-package.out"
        test -s Image.bz2
    ...
    (
        ...
        if scripts/prebuilt/verify-package.sh > "$tmp/verify-package-missing.out" 2> "$tmp/verify-package-missing.err"; then
...
EOF
    ...
    if (cd "$fixture" && PATH="$PWD/fakebin:$PATH" scripts/prebuilt/package.sh > "$output" 2>&1); then
