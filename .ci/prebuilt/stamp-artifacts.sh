...
# artifact restored from workflow cache, downloaded from release, or built in CI.
...
# materializer is the normal caller; the CLI remains for focused tests and
# manual CI artifact repair.
...
CI helper for recording .prebuilt/*.recipe-key stamps after raw artifacts are
downloaded or built by the CI materializer. With --clear, it removes selected
stamps. This helper accepts artifact classes, not build-image option flags.
USAGE
}
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --help|-h|help)
                ...
                ;;
            --*)
                echo "[!] stamp-artifacts.sh is a CI helper; unsupported helper option: $arg" >&2
            ...
            *)
                if prebuilt_class_plan_prefix "$arg" >/dev/null 2>&1; then
                    ...
                    echo "[!] stamp-artifacts.sh is a CI helper; unsupported artifact class: $arg" >&2
                    prebuilt_stamp_usage >&2
                    return 1
                fi
                ;;
