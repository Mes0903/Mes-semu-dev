...
#if SEMU_HAS(VIRTIOGPU)
static bool virtio_gpu_to_sdl_format(enum virtio_gpu_formats virtio_gpu_format,
                                     uint32_t *sdl_format)
{
    ...
    const struct vgpu_display_payload *payload)
{
    ...
    uint32_t sdl_format;
    if (!virtio_gpu_to_sdl_format(frame->format, &sdl_format)) {
    ...
    int hot_y)
{
    ...
    scanout->cursor_rect.x = x - hot_x;
    scanout->cursor_rect.y = y - hot_y;
    scanout->cursor_rect.w = (int) frame->width;
    scanout->cursor_rect.h = (int) frame->height;
    return true;
}
...
static void window_drain_display_queue(void)
{
    ...
    /* Drain published display commands, update only SDL-owned state, then
     * render each affected scanout once. This coalesces bursts such as
     * PRIMARY_SET + CURSOR_MOVE into a single display update per scanout.
     */
    while (vgpu_display_cmd_pop(&cmd)) {
        ...
        case VGPU_DISPLAY_CMD_CURSOR_MOVE:
            scanout->cursor_rect.x =
                cmd.u.cursor_move.x - scanout->cursor_hot_x;
            scanout->cursor_rect.y =
                cmd.u.cursor_move.y - scanout->cursor_hot_y;
            dirty_scanouts[cmd.scanout_id] = true;
            break;
        }
