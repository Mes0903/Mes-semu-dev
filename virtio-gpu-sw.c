                                               ...
                                               uint32_t *plen)
{
    ...
    if (request->width == 0 || request->height == 0) {
        fprintf(stderr, "%s(): invalid resource size %ux%u\n", __func__,
    ...
    if (vgpu_sw_get_resource_2d(request->resource_id)) {
        fprintf(stderr, "%s(): resource id %u already in use\n", __func__,
                request->resource_id);
        *plen = virtio_gpu_write_ctrl_response(
            vgpu, &request->hdr, response_desc,
            VIRTIO_GPU_RESP_ERR_INVALID_RESOURCE_ID);
    ...
    if (image_size > VGPU_SW_MAX_HOSTMEM ||
        ...
        fprintf(stderr, "%s(): image memory limit exceeded (%zu bytes)\n",
                __func__, image_size);
        vgpu_sw_destroy_resource_2d(res_2d);
        *plen =
            virtio_gpu_write_ctrl_response(vgpu, &request->hdr, response_desc,
                                               ...
                                               uint32_t *plen)
{
    ...
    if (!vgpu_sw_rect_fits(res_2d->width, res_2d->height, &request->r)) {
        fprintf(stderr, "%s(): invalid flush rect\n", __func__);
        *plen = virtio_gpu_write_ctrl_response(
            vgpu, &request->hdr, response_desc,
            VIRTIO_GPU_RESP_ERR_INVALID_PARAMETER);
        return;
                                                    ...
                                                    uint32_t *plen)
{
    ...
    if (!vgpu_sw_transfer_source_fits(req, res_2d)) {
        fprintf(stderr, "%s(): transfer source exceeds backing\n", __func__);
        *plen = virtio_gpu_write_ctrl_response(
            vgpu, &req->hdr, response_desc,
            VIRTIO_GPU_RESP_ERR_INVALID_PARAMETER);
        return;
    ...
    if (!vgpu_sw_copy_image_from_pages(req, res_2d)) {
        fprintf(stderr, "%s(): incomplete transfer from backing\n", __func__);
        *plen = virtio_gpu_write_ctrl_response(vgpu, &req->hdr, response_desc,
                                               VIRTIO_GPU_RESP_ERR_UNSPEC);
        return;
    }
    ...
    uint32_t *plen)
{
    ...
    if (backing_info->nr_entries == 0 ||
        ...
        fprintf(stderr, "%s(): invalid backing entry count %u\n", __func__,
    ...
    for (size_t i = 0; i < backing_info->nr_entries; i++) {
        if (pages[i].addr > UINT32_MAX) {
            fprintf(stderr, "%s(): page %zu addr_high non-zero\n", __func__, i);
            free(res_2d->iovec);
            res_2d->iovec = NULL;
            res_2d->page_cnt = 0;
            *plen = virtio_gpu_write_ctrl_response(
                                              ...
                                              uint32_t *plen)
{
    ...
    if (cursor->hdr.flags & VIRTIO_GPU_FLAG_FENCE)
        fprintf(stderr, "%s(): fenced cursor command is unsupported\n",
                __func__);
    ...
    if (res_2d->width == 0 || res_2d->height == 0 ||
        ...
        fprintf(stderr, "%s(): invalid cursor size %ux%u\n", __func__,
                res_2d->width, res_2d->height);
        *plen = 0;
        return;
    }
