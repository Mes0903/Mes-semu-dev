...
/* Immutable GL scanout metadata. The GL object remains owned by the future
 * window/GL renderer path; this bridge only transports the handle and source
 * view needed to retain the scanout on the consumer side.
 */
struct vgpu_display_gl_payload {
    uint32_t texture_id;
    uint32_t width, height;
...
enum vgpu_display_payload_type {
...
struct vgpu_display_payload {
    enum vgpu_display_payload_type type;
    struct vgpu_display_cpu_payload cpu;
    struct vgpu_display_gl_payload gl;
};
