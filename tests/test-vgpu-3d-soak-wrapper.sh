...
if ! grep -q '^test-vgpu-3d-soak-wrapper:' "${MAKEFILE}"; then
    printf '%s\n' 'expected Makefile target test-vgpu-3d-soak-wrapper'
    exit 1
fi
if ! grep -q '^test-host: .*test-vgpu-3d-soak-wrapper' "${MAKEFILE}"; then
    printf '%s\n' 'expected test-host to include test-vgpu-3d-soak-wrapper'
    exit 1
fi
