...
static void test_active_disable_after_actor_blob_dispatch_keeps_completion_live(
    void)
{
    uint32_t ram[512] = {0};
    emu_state_t emu;
    ...
    require_int(
        "notify actor blob create before active disable",
        vgpu.common.ops->notify_queue(vgpu.common.opaque, VIRTIO_GPU_CONTROLQ,
                                      vgpu.common.generation),
        0);
    wait_for_renderer_request(&queued);

    require_u32("actor blob queues command", queued.command_type,
                VIRTIO_GPU_CMD_RESOURCE_CREATE_BLOB);
    require_u64("actor blob renderer generation", queued.token.generation,
                vgpu.common.generation);
    require_u16("actor blob remains deferred before disable",
                load_u16(ram, 0x302), 0);
    require_u32("actor blob response untouched before disable", response->type,
                0);
    require_false("actor blob payload missing release hook",
                  queued.release_payload == NULL);
    ...
    require_u64("active actor blob keeps VirGL feature",
                vgpu.common.device_features & VIRTIO_GPU_F_VIRGL,
                VIRTIO_GPU_F_VIRGL);
    require_u64("active actor blob keeps context-init feature",
                vgpu.common.device_features & VIRTIO_GPU_F_CONTEXT_INIT,
                VIRTIO_GPU_F_CONTEXT_INIT);
    require_u64("active actor blob keeps resource-blob feature",
                vgpu.common.device_features & VIRTIO_GPU_F_RESOURCE_BLOB,
                VIRTIO_GPU_F_RESOURCE_BLOB);
    require_false("active actor blob keeps host-visible SHM present",
                  !vgpu.common.has_shm_region);
    require_u32("active actor blob keeps host-visible SHM id",
                vgpu.common.shm_region.id, VIRTIO_GPU_SHM_ID_HOST_VISIBLE);
    require_u64("active actor blob keeps host-visible SHM base",
    ...
    require_u64("active actor blob keeps renderer generation",
    ...
    require_int("queue actor blob renderer completion",
                vgpu_renderer_complete(&completion), true);
    virtio_gpu_drain_renderer_completions(&vgpu);

    require_u32("actor blob completion response", response->type,
                VIRTIO_GPU_RESP_OK_NODATA);
    require_u16("actor blob used idx after completion", load_u16(ram, 0x302),
                1);
    ...
    vgpu_renderer_debug_snapshot(&renderer_stats);
    require_u32("actor blob completion queue drained",
...
static void test_virgl_resource_flush_sees_pending_3d_scanout(void)
{
    uint32_t ram[1024] = {0};
    emu_state_t emu;
    virtio_gpu_state_t vgpu;
    ...
    struct virtio_gpu_set_scanout *scanout =
        (struct virtio_gpu_set_scanout *) ((uint8_t *) ram + 0x100);
    struct virtio_gpu_res_flush *flush =
        (struct virtio_gpu_res_flush *) ((uint8_t *) ram + 0x180);
    struct virtio_gpu_ctrl_hdr *response =
    ...
    g_virtio_gpu_backend.resource_create_3d(&vgpu, create_desc, &len);
    require_u32("3d create before pending flush is deferred", len,
                VIRTIO_GPU_RESPONSE_DEFERRED);
    require_int("3d create before pending flush queued",
                vgpu_renderer_pop_request(&queued), true);
    queued.release_payload(queued.payload);
    ...
    g_virtio_gpu_backend.set_scanout(&vgpu, scanout_desc, &len);
    require_u32("pending 3d set scanout is deferred", len,
                VIRTIO_GPU_RESPONSE_DEFERRED);
    require_u32("pending 3d set scanout is not committed",
                data->scanouts[0].primary_resource_id, 0);
    require_int("pending 3d set scanout queued",
                vgpu_renderer_pop_request(&queued), true);
    ...
    require_false("pending 3d scanout generation assigned",
                  scanout_generation == 0);
    queued.release_payload(queued.payload);

    flush->hdr.type = VIRTIO_GPU_CMD_RESOURCE_FLUSH;
    flush->resource_id = 74;
    flush->r.x = 0;
    flush->r.y = 0;
    flush->r.width = 320;
    flush->r.height = 240;
    flush_desc[0].addr = 0x180;
    flush_desc[0].len = sizeof(*flush);
    flush_desc[1].addr = 0x200;
    flush_desc[1].len = sizeof(*response);
    flush_desc[1].flags = VIRTIO_DESC_F_WRITE;
    vgpu.ctrl_dispatch.desc_head = 54;
    len = 0;

    g_virtio_gpu_backend.resource_flush(&vgpu, flush_desc, &len);
    require_u32("pending 3d scanout flush is deferred", len,
                VIRTIO_GPU_RESPONSE_DEFERRED);
    require_int("pending 3d scanout flush queued",
                vgpu_renderer_pop_request(&queued), true);
    struct vgpu_renderer_ctrl_payload *flush_payload = queued.payload;
    require_u32("pending 3d scanout flush command type", queued.command_type,
                VIRTIO_GPU_CMD_RESOURCE_FLUSH);
    require_u32("pending 3d scanout flush snapshots scanout id",
                flush_payload->scanout_id, 0);
    require_u64("pending 3d scanout flush snapshots generation",
                flush_payload->scanout_generation, scanout_generation);
    require_u32("pending 3d scanout flush snapshots resource",
                flush_payload->scanout.primary_resource_id, 74);
    require_u32("pending 3d scanout flush snapshots width",
                flush_payload->scanout.src_w, 320);
    queued.release_payload(queued.payload);
...
int main(void)
{
    ...
    test_virgl_resource_flush_sees_pending_3d_scanout();
