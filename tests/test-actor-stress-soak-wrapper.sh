...
if ! grep -q '^test-actor-stress-soak-wrapper:' "${MAKEFILE}"; then
    printf '%s\n' 'expected Makefile target test-actor-stress-soak-wrapper'
    exit 1
fi
if ! grep -q '^test-host: .*test-actor-stress-soak-wrapper' "${MAKEFILE}"; then
    printf '%s\n' 'expected test-host to include test-actor-stress-soak-wrapper'
    exit 1
fi
