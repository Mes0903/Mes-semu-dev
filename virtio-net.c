                                       ...
                                       uint32_t size)
{
    ...
    if (!vnet || !vnet->priv ||
        !virtio_net_config_range_valid(offset, size))
        return 0;
...
{
    struct pollfd rx = {usr->guest_to_host_channel[SLIRP_READ_SIDE], POLLIN,
                        0};
...
{
    return virtio_net_queue_fd_ready(vnet, VNET_QUEUE_TX)
               ? 0
               : SEMU_EVENT_WRITABLE;
}
...
{
    uint32_t token;
    uint32_t ready = SEMU_EVENT_READABLE | SEMU_EVENT_WRITABLE |
                     SEMU_EVENT_ERROR;
